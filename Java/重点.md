...
11. CMS（Concurrent Mark Sweep）收集器，是以获取最短回收停顿时间为目标的收集器，并发指的是与用户线程同时执行。设置JVM参数`-XX:+UseConcMarkSweepGC`后，开启ParNew(Young区)+CMS(Old区)+Serial Old(Old区备用)的收集器组合。步骤：初始标记、并发标记（和用户线程同时）、重新标记、并发清除（和用户线程同时）
