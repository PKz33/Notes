...
4. `java`程序属于抢占式调度，优先级高的线程优先执行。优先级相同，随机选择一个执行
5. 获取线程的名称：  
...
```
  ...
  // 此时，可能context对象还没有加载完成，变量ready已经为true，线程B直接跳出循环等待，执行do方法，结果会出现错误
```
6. `volatile`使用版本号机制避免指令重排  
- **原子变量**   
![](./Pics/并发编程原子性问题.png)   
